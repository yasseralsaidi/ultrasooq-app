...
import {
  useGetDynamicFormByID,
  useUpdateDynamicForm,
...
const EditForm = () => {
  ...
  const getDynamicFormID = useGetDynamicFormByID();
  const [getFormID, setFormID] = useState(null);
  ...
  const updateDynamicForm = useUpdateDynamicForm();
  const history = useHistory();

  useEffect(() => {
    const getFormByID = async () => {
      const payload = {
        id: query?.formID ? +query.formID : null,
      };
      const response = await getDynamicFormID.mutateAsync(payload);

      if (response?.status) {
        const formDataStructure = JSON.parse(response?.data?.formData);
        setFormID(response?.data?.id);
        setTemplateTitle(response?.data?.formName);
        setSectionListValues(formDataStructure);
        const newSectionCount = Object.keys(formDataStructure).length;
        setSectionSelected(true);
        activeSectionID.current = newSectionCount - 1;
      }
    };
  ...
  const handleRemoveFields = useCallback(
    (currentSectionID: any, currentNodeID: any) => {
      Swal.fire({
        title: "Warning! You're about to delete this field!",
        showDenyButton: true,
        confirmButtonText: "Yes, Delete",
  ...
  const handleDragStartSection = (e: any, index: any) => {
    e.dataTransfer.setData("index", index);
  };
  ...
  const rearrangeElements = useCallback(
    (sectionArray: any, draggedIndex: any, targetIndex: any) => {
      if (
        draggedIndex < 0 ||
        draggedIndex >= sectionArray.length ||
        targetIndex < 0 ||
  ...
  const handleFormSubmit = useCallback(() => {
    ...
    const submitForm = async () => {
      ...
      const response = await updateDynamicForm.mutateAsync(payload);
      if (response?.status) {
    ...
    submitForm();
  }, [getFormID, history, sectionListValues, templateTitle, updateDynamicForm]);
