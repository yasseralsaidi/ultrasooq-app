...
import { useAllServices } from "../../../apis/queries/services.queries";
      ...
      /> */}
            <div className="listingPages allAccountsPage">
                ...
                <div className="listingMain">
                        ...
                        <Table responsive>
                            ...
                            <tbody>
                                {servicesQuery.data && servicesQuery.data.data?.length > 0 ? (
                                    servicesQuery.data?.data?.map((item: any) => (
                                        <tr key={item.id}>
                                            {/* TODO:need thumbnail from backend */}
                                            <td>
                                                <Link
                                                    to={`/user/services/${item.id}`}
                                                    className="text-blue-600 hover:underline"
                                                >
                                                    <img
                                                        src={
                                                            item?.images?.[0]?.url &&
                                                                validator.isURL(item?.images?.[0]?.url)
                                                                ? item.images[0].url
                                                                : NoImagePlaceholder
                                                        }
                    ...
                    {servicesQuery.data?.totalCount > limit ? (
                        <Pagination
                            page={page}
                            setPage={setPage}
                            totalCount={servicesQuery.data?.totalCount}
                            limit={limit}
                        />
                    ) : null}
                </div>
